...
drop _merge
compress
save data/mergefile-csrhub-cstat-matched-cusip9-year.dta, replace
restore
...
replace conm=firm_cstat_man if conm==""
replace fyear=year if fyear==.

bysort conm fyear: gen N=_N
tab N
/*          N |      Freq.     Percent        Cum.
------------+-----------------------------------
          1 |      2,233       99.47       99.47
...
------------+-----------------------------------
      Total |      2,245      100.00
*/
drop if cusip=="CSMGACNO"
drop N

///	MERGE WITH CSTAT ON CONM-FYEAR
merge 1:m conm fyear using ///
	data/cstat-all-variables-for-all-cusip9-in-csrhub-and-kld-1990-2018.dta, ///
	update assert(1 2 3 4 5)
/*    Result                           # of obs.
    -----------------------------------------
    not matched                       115,743
        from master                     1,894  (_merge==1)
        from using                    113,849  (_merge==2)

    matched                               345
        not updated                         0  (_merge==3)
        missing updated                     0  (_merge==4)
        nonmissing conflict               345  (_merge==5)
    -----------------------------------------
*/
...
keep if _merge==5
compress
save data/mergefile-csrhub-cstat-matched-manual-cusip9-year.dta, replace

///	APPEND
append using data/mergefile-csrhub-cstat-matched-cusip9-year.dta
